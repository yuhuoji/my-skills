...
- one Markdown file
- one asset directory with only necessary files by default
- one zip that expands directly into the Markdown file and the asset directory
...
If the user does not specify naming, do not ask. Use the default pattern.
...
8. Package the Markdown and asset directory into a clean zip from inside the output directory so the archive does not add an extra parent folder.
